#!/usr/bin/env python3
"""Backtest #4: last 60s no-entry zone and 60s settlement protection."""
import json
import sqlite3
import time
        ...
        for timestamp, btc_price in btc_ticks:
            ...
            for trade in trades:
                ...
                if current_price >= tp_threshold:
                    ...
                    trade["final_reason"] = "TP"
                    trade["x"] = True
                    continue

                if stop_hit(trade, btc_change):
                    trade["raw_pnl"] += market_exit_pnl(
                        trade["direction"], trade["entry_yes"], yes_mid, trade["size"]
                    )
                    trade["explicit_exit_size"] = trade["size"]
                    trade["final_reason"] = "SL"
                    trade["x"] = True
...
def print_result(label, result, indent=""):
    print(f"{indent}{label}: PnL ${result.pnl:+.2f} | WR {result.win_rate:.1f}% | {result.trades} trades")
    print(
        f"{indent}  exits: TP {result.tp_exits}, SL {result.sl_exits}, "
        f"Settlement {result.settlement_exits}"
    )


def walk_forward(windows):
    fold_size = len(windows) // 4
    folds = [
        (windows[:fold_size], windows[fold_size : 2 * fold_size]),
        (windows[: 2 * fold_size], windows[2 * fold_size : 3 * fold_size]),
        (windows[: 3 * fold_size], windows[3 * fold_size :]),
    ]

    print("=== 3-fold walk-forward ===")
    for idx, (train, test) in enumerate(folds, start=1):
        train_result = simulate(train)
        test_result = simulate(test)
        print(f"Fold {idx}: train {len(train)} windows, test {len(test)} windows")
        print_result("Train", train_result, indent="  ")
        print_result("Test ", test_result, indent="  ")


def main():
    windows = load_all_windows()
    result = simulate(windows)
    ...
    print()
    print("=== Full backtest ===")
    print_result("240s/60s", result)
    print(
        "Baseline: "
        f"PnL ${BASELINE_PNL:+.2f} | WR {BASELINE_WR:.1f}% | {BASELINE_TRADES} trades"
    )
    print(
        "Delta: "
        f"PnL ${result.pnl - BASELINE_PNL:+.2f} | "
        f"WR {result.win_rate - BASELINE_WR:+.1f}pp | "
